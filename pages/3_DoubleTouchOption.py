...
from descriptions.black_scholes import model_description, single_touch_option_pricing_formula, double_touch_option_pricing_formula
from plots import european_option_plot
from utils.constants import *
from utils.enums import DoubleTouchOptionType
from utils.registry import get_price, get_greeks

color = "blue"
st.title(f":{color}[**Double Touch Option**]")
...
with col2:
    ...
    #option_type = st.selectbox(f":{color}[**Option Type**]", DoubleTouchOptionType.to_list(case="upper"))
    option_type = st.selectbox(
        f":{color}[**Option Type**]",
        options=list(DoubleTouchOptionType),
        format_func=lambda m: m.value.upper()
    )
    is_dot = option_type.lower() == "dot"
...
q /= 100.0
sigma /= 100.0
PaE = ["expiry" in p.lower() for p in pay_mode] if is_dot else ("expiry" in pay_mode.lower())

params = dict(option_type=option_type, S=S, T=T, r=r, q=q, sigma=sigma, Ll=Ll, Lh=Lh, rbt=rbt, PaE=PaE)
inst_name = "Double Touch Option"
...
if show_button:
    ...
    if price is not None:
        ...
        st.plotly_chart(
            european_option_plot.plot_price(model=model, instrument=inst_name, method=method, param=params, vlines=["Ll", "Lh"])
        )

        if selected_greeks:
            st.plotly_chart(
                european_option_plot.plot_greeks(model=model, instrument=inst_name, method=method, param=params,
                                                 selected=selected_greeks, vlines=["Ll", "Lh"])
            )
    else:
...
with st.expander("📚 Model Description"):
    model_description()
    double_touch_option_pricing_formula()
