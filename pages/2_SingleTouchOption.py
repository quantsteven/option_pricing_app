...
from descriptions.black_scholes import model_description, single_touch_option_pricing_formula
from plots import european_option_plot
from utils.constants import *
from utils.enums import SingleTouchOptionType
from utils.registry import get_price, get_greeks

color = "blue"
st.title(f":{color}[**Single Touch Option**]")
...
with col2:
    ...
    #option_type = st.selectbox(f":{color}[**Option Type**]", SingleTouchOptionType.to_list(case="upper"))
    option_type = st.selectbox(
        f":{color}[**Option Type**]",
        options=list(SingleTouchOptionType),
...
params = dict(option_type=option_type, S=S, T=T, r=r, q=q, sigma=sigma, L=L, rbt=rbt, PaE=PaE)
inst_name = "Single Touch Option"
...
if show_button:
    ...
    if price is not None:
        ...
        st.plotly_chart(
            european_option_plot.plot_price(model=model, instrument=inst_name, method=method, param=params, vlines=["L"])
        )

        if selected_greeks:
            st.plotly_chart(
                european_option_plot.plot_greeks(model=model, instrument=inst_name, method=method, param=params,
                                                 selected=selected_greeks, vlines=["L"])
            )
    else:
...
with st.expander("📚 Model Description"):
    model_description()
    single_touch_option_pricing_formula()
